...
공개 서버는 **2026학년도 1학기에 수집한 고려대학교 세종캠퍼스 빅데이터사이언스학부 과목 25개**를 사용합니다. 수집 당시의 강의계획서이며 최신 개설 정보로 갱신한 자료는 아닙니다. 실제 원문·정제 본문·벡터 DB는 GitHub에서 제외합니다.

## 서버에 전달하는 데이터

저장소 루트의 `data/processed/syllabus_texts.jsonl`에 본인이 사용할 수 있는 원본을 두고 `python scripts/prepare_hosted_data.py`를 실행합니다. 원본은 보존하고 연락처·로컬 경로를 제외한 서버용 파일을 `artifacts/hosted/syllabus_texts.jsonl`에 만듭니다. 공개 기록에는 문서 수와 파일 해시 등 검사 결과를 남깁니다.
...
Secret File은 서버 설정에 저장되어 실행 시 제공됩니다. 실행 중 임시 파일이 지워지는 무료 호스팅 특성과 구분합니다. 실제 재시작 후에도 같은 25개 자료와 해시를 독립 접속으로 확인했습니다. 개발자 PC나 방문자의 로그인으로 자료를 읽어 오는 구조가 아닙니다. [공개 서버 확인 기록](../evaluation/hosted-deployment-checks.json)
