...
**공개 웹 데모: 배포 진행 중** · [3분 시연 가이드](docs/demo.md) · [문서 모아보기](docs/README.md)
...
공개 예시는 직접 작성한 **가상 과목 6개**를 사용합니다. 아래의 로컬 데모는 API 키 없이 실행되며, 실제 대학의 개설 정보를 제공하지 않습니다.
...
| 검증 | 확인된 결과 |
|---|---|
| 자동 테스트 | 설정·캐시·대화 격리·화면·평가 지표 **72개 통과** |
| 실제 과목의 기존 질문 회귀 검사 | API와 답변 캐시를 끈 상태에서 **103/103개 조건 통과** |
| 검색 방식 진단 | 30문항의 근거 포함 과목 Recall@5 chunks: 고정 길이 **50.0%**, 구역별 **33.3%** |

회귀 검사는 정해진 기능 조건을 확인하며, 생성 답변 전체의 사실성 점수는 아닙니다. 검색 진단에서는 구역별 분할만으로 개선되지 않는 결과도 기록했습니다. 실제 API 연결 검증과 공개 배포는 진행 중이며, [평가 보고서](docs/evaluation.md)에서 실험 조건·실패 사례·재현 범위를 확인할 수 있습니다.
...
[localhost:7861](http://127.0.0.1:7861)을 열면 됩니다. 종료는 실행 창에서 `Ctrl+C`입니다. 실제 데이터·Gemini·Chroma 연결 및 테스트 실행은 [실행 안내](docs/running.md)를 참고하세요.
