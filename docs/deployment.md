...
새 대화 화면 배포 후 GitHub의 별도 실행 환경에서 **4개 검사가 모두 통과**했습니다. 기존 API의 근거 질문·시간표·범위 밖 안내 3개에 새 화면용 JSON 시간표 요청 1개를 추가했습니다. 검사 클라이언트는 로그인·쿠키·API 키 없이 공개 주소만 사용했습니다. 근거 질문은 캐시를 쓰지 않은 실제 Gemini 생성 응답으로 확인했습니다. [최신 독립 실행](https://github.com/JunH14/sugang-mate/actions/runs/34442016281) · [새 화면 검사 기록](../evaluation/chat-ui-checks.json)

브라우저에서는 BDSC205의 SAS 근거 질문, BDSC201·BDSC203 평가 비교와 전공필수 후속 질문, 대화 초기화를 확인했습니다.

이전 공개본에서는 서버를 재시작한 뒤 GitHub가 제공하는 별도 Ubuntu·Python 3.12 환경에서 **근거 질문·시간표·범위 밖 안내 3개가 모두 통과**했습니다. 검사 클라이언트는 로그인·쿠키·API 키 없이 공개 주소만 사용했고 BDSC205 질문은 캐시를 쓰지 않은 실제 Gemini 응답이었습니다. 데이터 25개와 해시 일치를 확인했으며 비밀 데이터 파일의 직접 다운로드 요청은 HTTP 403으로 차단됐습니다. [성공한 독립 실행](https://github.com/JunH14/sugang-mate/actions/runs/34437384036) · [배포 확인 집계](../evaluation/hosted-deployment-checks.json)
