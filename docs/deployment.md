...
공개 데모를 **2026학년도 1학기에 수집한 실제 과목 25개와 Gemini API**로 전환합니다. 최신 개설 정보를 다시 수집한 서비스는 아닙니다. 방문자는 설치·API 키·학교 로그인 없이 주소만 열어 사용할 수 있으며, 서비스는 개발자의 PC와 독립된 Render 서버에서 실행됩니다. 실제 과목으로의 전환 후 외부 접속 검사는 아직 확인 단계입니다.
...
배포 후에는 개발자 PC 밖의 GitHub Actions에서 API 키 없이 공개 주소의 실제 과목 조회·비교·후속 질문을 확인합니다. [공개 서버 검사](https://github.com/JunH14/sugang-mate/actions/workflows/hosted-demo.yml)는 수동 실행 방식이며, 실제 과목 전환에 대한 결과는 확인 후 기록합니다.
