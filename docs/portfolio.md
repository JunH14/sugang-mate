...
여러 강의계획서를 직접 열어 평가방식·수업 내용·시간표를 확인해야 하는 불편함을 줄이기 위해, 과목 정보 조회와 문서 기반 질의응답을 결합한 서비스를 구현했다. 대상 데이터는 2026학년도 1학기 고려대학교 세종캠퍼스 빅데이터사이언스학부 강의계획서다.
...
![공개 예시의 과목 비교](assets/demo-comparison.png)
...
| 공개 실행 | 직접 작성한 6개 가상 과목으로 API 키 없는 실행 제공 |
...
현재 공개본은 지원 가능한 데이터 범위가 제한된 프로토타입이다. 서비스 구현 능력과 함께 데이터 품질 판단, 지표 해석, 실패를 드러내는 실험 설계를 보여주는 데 초점을 두었다.

## 직무별 소개 문구

### AI 서비스 개발 지원

강의계획서와 과목 메타데이터를 활용한 수강 상담 서비스를 구현했습니다. 질문 유형에 따라 구조화 조회와 문서 검색을 연결하고, 대화 맥락·출처 표시·오류 대응을 구성했습니다. 공개본에는 API 키 없는 실행 환경과 캐시·대화 격리 테스트를 추가했으며, 기존 기능 회귀와 신규 검색 진단을 구분해 결과를 보고했습니다.
