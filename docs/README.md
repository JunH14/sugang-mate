...
- **[3분 시연](demo.md)** — 대화 화면 사용법, 답변별 근거 확인, 과목 비교·후속 질문의 입력과 예상 결과입니다.
...
공개 화면은 대화와 입력창을 중심으로 구성합니다. 과거 답변의 근거도 그 답변에서 다시 펼칠 수 있으며 새 대화·과목 목록·서비스 설명은 메뉴에서 찾을 수 있습니다. PC·모바일 첫 화면은 [프로젝트 소개 화면](../README.md#이렇게-사용합니다)에서 볼 수 있습니다.
...
- **[최신 공개 서버 외부 검사](https://github.com/JunH14/sugang-mate/actions/runs/34442016281)** — GitHub의 별도 환경에서 로그인·방문자 API 키 없이 근거 질문·시간표·범위 밖 안내와 새 채팅 화면의 시간표 요청까지 4개 검사를 통과했습니다. 캐시를 쓰지 않은 실제 Gemini 생성도 확인했습니다.
...
- [최신 대화 화면·외부 접속 검사](../evaluation/chat-ui-checks.json): PC·모바일 화면과 대화·근거·입력 동작, 별도 환경의 4개 검사
- [이전 실제 25개 공개 서버 확인](../evaluation/hosted-deployment-checks.json): [외부 접속 3개 검사](https://github.com/JunH14/sugang-mate/actions/runs/34437384036), 브라우저 시연, 재시작 후 같은 25개 자료·파일 해시 유지, 파일 다운로드 차단
