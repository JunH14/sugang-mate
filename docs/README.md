...
로컬 데모는 가상 과목 6개로 동작합니다. 실제 Gemini 생성·임베딩 연결은 검증했으며, 공개 웹 데모의 주소와 상태는 [프로젝트 첫 화면](../README.md)에 안내합니다.
...
현재 저장된 검증 기록은 자동 테스트 **72개**, 실제 과목의 오프라인 기능 회귀 **103/103개** 조건 통과와 실제 API **16개 흐름** 확인입니다. 생성 답변 12개는 로컬 원문과 대조했습니다. 검색 진단 30문항에서는 구역별 분할이 고정 길이 분할보다 나은 결과를 보이지 않았습니다. 이 수치는 실사용 효과나 독립적인 답변 정확도를 뜻하지 않습니다.
